...
export const getSecret = (secretName: string, envVarName: string) => {
  ...
  if (secretPath && fs.existsSync(secretPath)) {
    try {
      ...
      throw new Error(
        `CRITICAL ERROR: Secret is missing! Checked environment variables: ${secretName}_FILE and ${envVarName}. Application cannot start.`
      );
    }
  }
  ...
  throw new Error(
    `CRITICAL ERROR: Secret ${secretName} is missing! Application cannot start.`
  );
};
